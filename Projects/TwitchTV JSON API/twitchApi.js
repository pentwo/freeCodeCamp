...
fetch(endPoint1 + streamer.join(','), {
  ...
  .then((data) => {
    ...
    for (let i = 0; i < streamerInfo.length; i++) {
      ...
      html =
        '<div class="card col-md-3 px-0 my-2" >' +
        ...
        streamerInfo[i].display_name +
        '</a></h5>' +
        '<block class="badge badge-secondary status ' +
        streamerInfo[i].name +
        '">Offline</block>' +
        '<p class="card-text streaming ' +
        streamerInfo[i].name +
        'playing">' +
        '</p>' +
        '</div></div>';
        ...
        for (var i = 0; i < data.streams.length; i++) {
          for (var j = 0; j < streamerInfo.length; j++) {
            if (data.streams[i].channel._id == streamerInfo[j]._id) {
              ...
              $('.' + streamerInfo[j].name + 'playing').text(
                data.streams[i].channel.game
              );
            }
          }
        }
